...
const Sidebar = () => {
  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      if (isOpen && !event.target.closest(".sidebar") && !event.target.closest(".menu-btn")) {
        setIsOpen(false);
      }
    };
  ...
  return (
    <>
      ...
      <button
        className="lg:hidden fixed top-4 left-4 bg-gray-800 text-white p-2 rounded-lg z-50 menu-btn"
        onClick={(e) => {
